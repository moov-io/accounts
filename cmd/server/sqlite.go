...
var (
	...
	migrations = []string{
		...
		`create table if not exists customers_phones(customer_id, number, valid, type);`,
		`create table if not exists customers_addresses(customer_id, type, address1, address2, city, state, postal_code, country, validated, active, created_at datetime, deleted_at datetime);`,
